...
Requirements:
  pip install transformers torch datasets accelerate scikit-learn numpy

How to train:
  python models/model_c_transformer.py train --train data/data_splits/train.json --val data/data_splits/val.json --test data/data_splits/test.json --output_dir models/artifacts_c

How to predict:
  python models/model_c_transformer.py predict --input test_unlabeled.json --output submission_c.json --model_path models/artifacts_c/best_model
"""
...
# Constants
MODEL_NAME = 'distilbert-base-uncased' # Chosen for speed/efficiency in this environment
SEED = 42
MAX_LENGTH = 256 # Covers most Reddit posts while remaining efficient
...
def train_mode(args):
    ...
    # 2) Label Encoding
    le = LabelEncoder()
    all_labels = [item['label'] for item in train_raw]
    le.fit(all_labels)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Model C - Transformer Fine-tuning")
    ...
    train_parser.add_argument('--output_dir', default='models/artifacts_c')

    predict_parser = subparsers.add_parser('predict')
    predict_parser.add_argument('--input', required=True)
    predict_parser.add_argument('--output', default='submission_c.json')
    predict_parser.add_argument('--model_path', default='models/artifacts_c/best_model')
